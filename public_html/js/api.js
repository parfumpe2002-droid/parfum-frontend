(() => {
    ...
    async function request(path, options = {}) {
        ...
        try {
            ...
            if (error.name === "AbortError") throw new Error("El servidor está tardando demasiado en responder");
            throw error;
        } finally {
    ...
    function productUrl(product, options = {}) {
        ...
        if (product?.slug) query.set("slug", product.slug);
        else query.set("id", product?.id ?? product?.productoId ?? "");
        if (options.decant) query.set("decant", "1");
