(() => {
    ...
    const startInDecant = params.get("decant") === "1";
    ...
    let mode = startInDecant ? "DECANT" : "BOTELLA";
    ...
    function ensureSelections() {
        ...
        if (mode === "DECANT" && !product.decantDisponible) mode = "BOTELLA";
    }
    ...
    function renderModeTabs() {
        return `
            <div class="purchase-mode-tabs" role="tablist" aria-label="Tipo de compra">
                <button type="button" class="${mode === "BOTELLA" ? "active" : ""}" data-mode="BOTELLA">
                    <i class="fa-solid fa-bottle-droplet"></i>
                    <span><b>Botella</b><small>Presentación original</small></span>
                </button>
                <button type="button" class="${mode === "DECANT" ? "active" : ""}" data-mode="DECANT" ${product.decantDisponible === false ? "disabled" : ""}>
                    <i class="fa-solid fa-vial"></i>
                    <span><b>Pedir en decant</b><small>3 ml, 5 ml o 10 ml</small></span>
                </button>
            </div>`;
    }
    ...
    function renderProduct() {
        ...
        document.title = `${product.nombre} | Parfum`;
        grid.innerHTML = `
            ...
            <div class="detail-info">
                ...
                ${renderModeTabs()}
                ${mode === "DECANT" ? renderDecantOptions() : renderBottleOptions()}
                <div class="selected-variant-summary">
                    <div><small>${mode === "DECANT" ? "Decant seleccionado" : "Presentación seleccionada"}</small><b>${esc(variant?.etiqueta || variant?.envaseNombre || "Presentación estándar")}</b></div>
                ...
                <div class="button-row detail-buy-row">
                    ...
                    <button id="addCart" class="primary-btn" ${purchasable ? "" : "disabled"}>
                        <i class="fa-solid fa-bag-shopping"></i> ${purchasable ? `Agregar ${mode === "DECANT" ? "decant" : "al carrito"}` : "Próximamente"}
                    </button>
                ...
                ${mode === "DECANT" ? `<p class="decant-explanation"><i class="fa-solid fa-circle-info"></i> El decant contiene la fragancia del perfume seleccionado en el envase mostrado. Puedes guardar distintos tamaños y diseños por separado en carrito y favoritos.</p>` : ""}
        ...
        grid.querySelectorAll("[data-mode]").forEach(button => button.addEventListener("click", () => {
            mode = button.dataset.mode;
            quantity = 1;
            renderProduct();
            if (mode === "DECANT") document.getElementById("decantOptions")?.scrollIntoView({behavior:"smooth", block:"nearest"});
        }));
    ...
    async function load() {
        ...
        try {
            ...
            status.textContent = "Servidor iniciando. Mostrando la ficha local de respaldo.";
        }
