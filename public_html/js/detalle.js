(() => {
    ...
    function currentImage() {
        // El cliente quiere ver siempre el frasco/perfume real a la izquierda.
        // La presentación del decant se elige y se previsualiza en la columna derecha.
        return ParfumAPI.image(product);
    }
    ...
    function updateStructuredData() {
        if (!product) return;
        const prices = decants().map(item => Number(item.precio || 0)).filter(value => value > 0);
        const hasStock = decants().some(item => Number(item.stock || 0) > 0 && Number(item.precio || 0) > 0);
        const data = {
            "@context": "https://schema.org",
            "@type": "Product",
            name: `Decant ${product.nombre}`,
            brand: {"@type": "Brand", name: product.marca || "Parfum"},
            description: `Compra decants de ${product.nombre} de ${product.marca} en 3, 5, 10, 20 o 30 ml en Parfum Perú.`,
            image: currentImage(),
            url: `https://parfum.com.pe/decants/${product.slug}/`
        };
        if (prices.length) {
            data.offers = {
                "@type": "AggregateOffer",
    ...
    function renderProduct() {
        ...
        grid.innerHTML = `
            <div class="detail-image-wrap">
                <div class="detail-image detail-image-bottle">
                    <span class="detail-image-label"><i class="fa-solid fa-bottle-droplet"></i> Perfume original</span>
                    <img id="detailImage" src="${esc(currentImage())}" alt="Frasco de ${esc(product.nombre)} de ${esc(product.marca)}">
                </div>
            ...
            <div class="detail-info">
                ...
                <div class="selected-variant-summary">
                    <div class="selected-decant-preview">
                        <img src="${esc(ParfumAPI.variantImage(product, variant, "DECANT"))}" alt="Decant seleccionado" onerror="this.src='imagen/decants/decant-10ml-premium.png'">
        ...
        document.getElementById("detailImage").addEventListener("error", event => {
            event.target.src = product.fallbackImage || "imagen/perfumes/perfume-default.png";
        }, {once:true});
