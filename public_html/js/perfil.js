(() => {
    ...
    document.getElementById('passwordForm').onsubmit=async e=>{e.preventDefault();try{await ParfumAPI.request('/usuarios/me/password',{method:'PUT',body:{actual:document.getElementById('currentPassword').value,nueva:document.getElementById('newPassword').value}});e.currentTarget.reset();ParfumAPI.clearSession();msg('passwordMessage','Contraseña actualizada. Por seguridad, vuelve a iniciar sesión.');ParfumAPI.toast('Contraseña actualizada');setTimeout(()=>location.href='login.html',1400)}catch(err){msg('passwordMessage',err.message,'error')}};
    load();
})();
