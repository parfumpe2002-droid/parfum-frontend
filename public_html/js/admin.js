(() => {
    ...
    const KEEP_ALIVE_KEY = "parfum_admin_keep_render_awake";
    const KEEP_ALIVE_INTERVAL_MS = 9 * 60 * 1000;
    let keepAliveTimer = null;
    let keepAliveBusy = false;

    function keepAliveEnabled() {
        return sessionStorage.getItem(KEEP_ALIVE_KEY) === "1";
    }

    function setKeepAliveUi(enabled, state = enabled ? "active" : "off", detail = "") {
        const button = $("renderKeepAliveToggle");
        const badge = $("renderKeepAliveStatus");
        const last = $("renderKeepAliveLastPing");
        if (!button || !badge) return;
        button.innerHTML = enabled
            ? '<i class="fa-solid fa-power-off"></i> Dejar que Render se apague'
            : '<i class="fa-solid fa-bolt"></i> Mantener Render activo';
        button.classList.toggle("danger-btn", enabled);
        button.classList.toggle("secondary-btn", !enabled);
        badge.className = `status-pill ${state === "error" ? "inactive" : state === "checking" ? "pending" : enabled ? "active" : "inactive"}`;
        badge.innerHTML = state === "checking"
            ? '<i class="fa-solid fa-spinner fa-spin"></i> Comprobando'
            : state === "error"
                ? '<i class="fa-solid fa-triangle-exclamation"></i> Sin respuesta'
                : enabled
                    ? '<i class="fa-solid fa-circle-check"></i> Activo'
                    : '<i class="fa-solid fa-power-off"></i> Desactivado';
        if (last && detail) last.textContent = detail;
    }

    async function pingRender({silent = false} = {}) {
        if (!keepAliveEnabled() || keepAliveBusy) return;
        keepAliveBusy = true;
        if (!silent) setKeepAliveUi(true, "checking", "Comprobando el backend...");
        try {
            const health = await ParfumAPI.request("/health", {auth:false, timeout:20000});
            const now = new Date().toLocaleTimeString("es-PE", {hour:"2-digit", minute:"2-digit"});
            setKeepAliveUi(true, "active", `Última señal: ${now} · backend ${health?.version || "activo"}.`);
        } catch (error) {
            const now = new Date().toLocaleTimeString("es-PE", {hour:"2-digit", minute:"2-digit"});
            setKeepAliveUi(true, "error", `Último intento: ${now} · ${error.message}`);
        } finally {
            keepAliveBusy = false;
        }
    }
    ...
    function syncKeepAlive() {
        ...
        const enabled = keepAliveEnabled();
        setKeepAliveUi(enabled);
        if (!enabled) return;
        pingRender();
        keepAliveTimer = setInterval(() => pingRender({silent:true}), KEEP_ALIVE_INTERVAL_MS);
    }

    $("renderKeepAliveToggle")?.addEventListener("click", () => {
        const next = !keepAliveEnabled();
        sessionStorage.setItem(KEEP_ALIVE_KEY, next ? "1" : "0");
        syncKeepAlive();
        ParfumAPI.toast(next
            ? "Render se mantendrá activo mientras esta pestaña de Admin siga abierta."
            : "Mantenimiento desactivado. Render podrá dormirse si no recibe tráfico.");
    });

    window.addEventListener("focus", () => {
        if (keepAliveEnabled()) pingRender({silent:true});
    });
    document.addEventListener("visibilitychange", () => {
        if (!document.hidden && keepAliveEnabled()) pingRender({silent:true});
    });
